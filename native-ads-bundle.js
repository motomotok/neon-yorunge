(() => {
  ...
  async function requestAndShowConsentIfNeeded() {
    try {
      const info = await AdMob.requestConsentInfo({ debugGeography: void 0 });
  ...
  function init() {
    ...
    initPromise = requestAndShowConsentIfNeeded().then(() => AdMob.initialize({ initializeForTesting: ADS_TEST_MODE })).then(() => {
      preloadInterstitial();
      preloadRewarded();
    }).catch(() => {
    });
  ...
  async function showInterstitial(onClose) {
    ...
    const now = Date.now();
    if (now - lastInterstitialAt < MIN_INTERSTITIAL_GAP_MS) {
      onClose && onClose();
      return;
    }
    try {
